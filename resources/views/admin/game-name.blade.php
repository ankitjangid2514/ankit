...
<div class="main-content">	<div class="page-content">
    <div class="container-fluid">
        ...
        <!-- Zero Configuration  Starts-->
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title d-flex align-items-center justify-content-between">Game Name List			  <a class="btn btn-primary btn-sm btn-float" href="#addstarlinegameModal" role="button" data-toggle="modal">Add Game </a></h4>
    ...
    <div class="modal-dialog modal_right_side" role="document">
        ...
        <div class="modal-body">
            <form class="theme-form" method="post" action="{{route('gameName_insert')}}">
                @csrf
                <div class="row">
    ...
    <div id="editPopup" class="popup" style="display: none;">
        <h5>Edit Game</h5>
        <form id="editGameForm" method="post" action="{{route('edit_market')}}">
            @csrf
            <input type="hidden" name="game_id" value="">
            <div class="form-group">
                <label for="game_name">Game Name</label>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.reject-btn', function() {
                ...
                if (confirm('Are you sure you want to Inactive this market ?')) {
                    $.ajax({
                        url: "{{ route('inactive_market') }}", // Your route to handle delete
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: id
            ...
            $(document).on('click', '.accept-btn', function() {
                ...
                if (confirm('Are you sure you want to Active this market ?')) {
                    $.ajax({
                        url: "{{ route('active_market') }}", // Your route to handle delete
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: id
            ...
            $('#mytable tbody').on('click', '.btn-edit', function() {
                ...
                // $('#editGameForm input[name="game_name_hindi"]').val(nameHindi);
            ...
            $('#mytable tbody').on('click', '.btn-delete', function() {
                ...
                if (confirm('Are you sure you want to Delete this market ?')) {
                    $.ajax({
                        url: "{{ route('delete_market') }}", // Your route to handle delete
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: id
