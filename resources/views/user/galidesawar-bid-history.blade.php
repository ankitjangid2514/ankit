    ...
    @if($data->isEmpty()) <!-- Check if data is empty -->
        <div class="row">
            <div class="col-12 text-center">
                <p style="font-size: 18px; color: #d90707;">No Bid History Available</p> <!-- Message when no data -->
            </div>
        </div>
    @else
        <div class="row" style="border-bottom: 1px var(--primary) solid;">
            @foreach ($data as $bid)
                @php
                    $dateFromDB = $bid->desawar_bid_date;  // Example date from your database
                    ...
                    <div class="row">
                        <div class="col-11">
                            <p class="game_name" style="margin-top: 8px; font-size: 14px; font-family: system-ui;">
                                {{$bid->desawar_name}} ({{$bid->gtype}}) - {{$bid->digit}}
                            </p>
